...
import companiesData from "../company-data.json";

function App() {
  const [selectedCity, setSelectedCity] = useState('All');
  const [searchQuery, setSearchQuery] = useState('');

  const cities = ['All', ...new Set(companiesData.companies.map(c => c.city).filter(Boolean))];
  ...
  return (
    <div className="relative isolate bg-white px-6 py-16 sm:py-20 lg:px-8 min-h-screen">
...
function CompanyCard({ company }) {
  ...
  const hasTechData =
    company.tech_stack &&
    Object.values(company.tech_stack).some(arr => Array.isArray(arr) && arr.length > 0);
