...
// قائمة الجمل التي ستظهر في تأثير الكتابة
const phrases = [
...
// تعريف البطاقات العائمة التي تظهر حول الهيرو
const floatingCards = [
...
export default function Hero() {
  // حالات التحكم في تأثير الكتابة
  const [typed, setTyped] = useState('')        // النص المعروض حالياً
  const [phraseIdx, setPhraseIdx] = useState(0) // رقم الجملة الحالية
  const [charIdx, setCharIdx] = useState(0)     // رقم الحرف الحالي
  const [isDeleting, setIsDeleting] = useState(false) // هل نحن نحذف النص؟

  // تأثير الكتابة: يضيف أو يحذف الحروف تدريجياً
  useEffect(() => {
    ...
    if (!isDeleting) {
      ...
      if (charIdx <= current.length) {
        ...
        // بعد اكتمال الجملة، الانتظار ثم البدء بالحذف
      ...
      if (charIdx >= 0) {
        ...
        // عند اكتمال الحذف، الانتقال للجملة التالية
        setIsDeleting(false)
    ...
    // تنظيف الـ timeout عند تحديث الحالة
  ...
  return (
    <section className="relative z-5 min-h-[calc(100vh-80px)] flex flex-col items-center justify-center text-center px-4 py-16">
      ...
      <div className="inline-flex items-center gap-2 bg-indigo-50 border border-indigo-200 text-indigo-600 px-4 py-1.5 rounded-full text-xs tracking-wide mb-8"
        ...
        <span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-pulse-dot" /> {/* نقطة متحركة */}
